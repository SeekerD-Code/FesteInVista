...
export function creaCardEvento(evento, isPassato = false) {
    ...
    // Pulizia dell'ID per evitare conflitti con attributi HTML e caratteri di escape/virgolette
    const rawId = evento.id || evento.nome_rilevato || evento.nome || evento.titolo || '';
    const eventoId = String(rawId).replace(/["']/g, '').trim();

    const preferiti = JSON.parse(localStorage.getItem('festeinvista_preferiti') || '[]');
    const preferitiPuliti = preferiti.map(id => String(id).replace(/["']/g, '').trim());

    const isPreferito = preferitiPuliti.includes(eventoId);
    ...
    return `
        <div class="card card-evento ${classePassato}" style="position: relative;">
            ...
            <div class="card-content-wrapper">
                ...
                <button class="btn-preferito-overlay ${cuoreClass}" data-id="${eventoId}" title="${isPreferito ? 'Rimuovi dai preferiti' : 'Aggiungi ai preferiti'}">
                    ${webpCuore}
                </button>
...
export function initCustomCalendar() {
    ...
    for (let day = 1; day <= totalDays; day++) {
        ...
        dayCell.addEventListener('click', (e) => {
            ...
            initCustomCalendar();
            if (typeof applicaFiltriMappa === 'function') {
                applicaFiltriMappa();
            }
        });
